...
#ifndef INC_SBUS4T12K_H_
#define INC_SBUS4T12K_H_
...
void convertReceiveData() {
		...
		for(int i = 0; i < 7; i++) {
			if((swSelectData[i] == RVR)||(swSelectData[i]==LVR)) {
				float vr_buf = 0.0f;
				if(CH[8] > (stickNeutralValue + 20)) vr_buf = ((CH[8] - stickNeutralValue) / (float)(stickMaximumValue - stickNeutralValue));
				else if(CH[8] < (stickNeutralValue - 20)) vr_buf = -((CH[8] - stickNeutralValue) / (float)(stickMinimumValue - stickNeutralValue));
				else vr_buf = 0.0f;
		...
		for(int j=0;j<2;j++){
			switch(CH[j + 12]) {
				case 0x6A0:
					switchValue[swSelectData[j+5]] = HIGH;
					break;
				case 0x790:
					switchValue[swSelectData[j+5]] = HIGH;
					break;
				case 0x400:
					switchValue[swSelectData[j+5]] = NEUTRAL;
					break;
				case 0x160:
					switchValue[swSelectData[j+5]] = LOW;
				case 0x070:
					switchValue[swSelectData[j+5]] = LOW;
					break;
				default:
...
void resetAllData(){
	...
	for(int i = 0; i < 14; i++) switchValue[i] = NEUTRAL;
	volumeValue = 0.0f;
}

void sbus(){
	stickMaximumValue = 0x06A0;
	stickNeutralValue = 0x0400;
	stickMinimumValue = 0x0160;
...
void setup(int ch5,int ch6,int ch7,int ch8,int ch9,int ch13,int ch14) {
	swSelectData[0] = ch5;
	swSelectData[1] = ch6;
	swSelectData[2] = ch7;
	swSelectData[3] = ch8;
